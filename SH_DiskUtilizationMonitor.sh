...
function check_server {
    ...
    if [ "$status" == "OK" ]; then
        echo "<tr><td style='height: 40px; background-color:lavender;'><strong>$hostname</strong></td><td style='color:black; background-color:lightgreen; height: 40px;'><strong>$status</strong></td>" >> "$output_file"
        ...
        echo "<tr style='background-color:lightcoral;'><td style='height: 40px; background-color:lavender;'><strong>$hostname</strong></td><td style='color:black; background-color:lightcoral; height: 40px;'><strong>$status</strong></td><td style='background-color:lightcoral; height: 40px;'>N/A</td><td style='background-color:lightcoral; height: 40px;'>N/A</td></tr>" >> "$output_file"
    fi
}
 
# Create directory for the output file if it is not there
...
echo "<table border='1' style='border-collapse: collapse;'><tr><th style='color:black; background-color:yellow; width: 100px; height: 40px;'> &nbsp; IP Address &nbsp; </th><th style='color:black; background-color:yellow; width: 70px; height: 40px;'> &nbsp; Status &nbsp; </th><th style='color:black; background-color:yellow; width: 70px; height: 40px;'> &nbsp; Disk Utilization Before &nbsp; </th><th style='color:black; background-color:yellow; width: 70px; height: 40px;'> &nbsp; Disk Utilization After &nbsp; </th></tr>" >> "$output_file"
 
 
 
# Read ip_addresses from file and check status using a for loop
for ip_address in $(cat "$input_file"); do
    check_server "$ip_address"
done
 
# Add footer to the HTML file
