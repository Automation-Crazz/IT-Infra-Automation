    ...
    def collect_windows_data(self):
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                return None, f"PowerShell error: {result.stderr}"
        except Exception as e:
...
"""
    ...
    def parse_nmap(self, content):
        ...
        try:
            ...
            st.error(f"Error parsing Nmap file: {str(e)}")
    ...
    def parse_openvas(self, content):
        ...
        try:
            ...
            st.error(f"Error parsing OpenVAS file: {str(e)}")
...
def main():
    ...
    if llm_status:
        ...
        st.sidebar.warning(f"⚠️ {llm_message}")
        st.sidebar.info("Install Ollama and run: `ollama pull phi`")
...
def show_windows_analysis():
    ...
    if st.button("Collect System Data"):
        with st.spinner("Collecting Windows system information..."):
            ...
            if st.button("Analyze Risks") and risk_input:
                if analyzer.check_ollama_status()[0]:
...
def show_llm_analysis():
    ...
    if analysis_type == "Custom Security Query":
        ...
        if st.button("Check Compliance") and system_description:
            prompt = f"""
        ...
        if st.button("Analyze Risk") and current_security:
            risk_context = {
        ...
        try:
            ...
            if st.button("Retrain Model"):
                if os.path.exists(os.path.join(analyzer.data_dir, "vulnerabilities.json")):
                    ...
                    st.error("No training data available. Please upload scan files first.")
